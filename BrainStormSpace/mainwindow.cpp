...
void MainWindow:: on_pdf_employee_clicked()
{
    QString strStream;
        ...
        const int rowCount = ui->tableView_employee->model()->rowCount();
        const int columnCount = ui->tableView_employee->model()->columnCount();
        ...
        out << "<thead><tr bgcolor=#f0f0f0>";
        for (int column = 0; column < columnCount; column++)
            if (!ui->tableView_employee->isColumnHidden(column))
                out << QString("<th>%1</th>").arg(ui->tableView_employee->model()->headerData(column, Qt::Horizontal).toString());
        out << "</tr></thead>\n";
        ...
        for (int row = 0; row < rowCount; row++) {
            ...
            for (int column = 0; column < columnCount; column++) {
                if (!ui->tableView_employee->isColumnHidden(column)) {
                    QString data = ui->tableView_employee->model()->data(ui->tableView_employee->model()->index(row, column)).toString().simplified();
                    out << QString("<td bkcolor=0>%1</td>").arg((!data.isEmpty()) ? data : QString("&nbsp;"));
                }
            }
            out << "</tr>\n";
